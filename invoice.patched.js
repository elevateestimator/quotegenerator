/* ===========================================================
   ...
   - Reuses the quote app's PDF pipeline
   - Adds Paid-to-Date and Amount Due logic
   - Includes a status badge (PAID / DUE / DUE TODAY / PAST DUE)
   =========================================================== */
...
function makeRow(data = {}) {
  ...
  tr.innerHTML = `
    ...
    <td class="center"><input type="checkbox" class="taxable" ${data.taxable ? 'checked' : 'checked'}></td>
...
/* ===== Status badge ===== */
function updateStatusBadge(currentDue = null) {
  const badge = $('#status-badge');
  if (!badge) return;

  const dueStr = $('[data-bind="invoice_due"]')?.value;
  if (!dueStr) { badge.textContent = 'DUE'; badge.className = 'status-badge due'; return; }
  ...
  const dueAmt = currentDue == null ? parseNum($('#amount-due')?.textContent) : currentDue;
  if (dueAmt <= 0.0001) { badge.textContent = 'PAID'; badge.className = 'status-badge paid'; return; }
  ...
  if (d.getTime() < now.getTime()) {
    ...
    badge.className = 'status-badge today';
  } else {
...
function recalcAll() {
  ...
  // Paid & Due
  const paid = parseNum($('#paid').value);
  const due  = Math.max(0, grand - paid);
  $('#amount-due').textContent = formatMoney(due);

  updateStatusBadge(due);
}
...
function buildPrintClone() {
  ...
  if (hst)   contactParts.push('HST ' + hst);
  ...
  if (header && oldBrandRow) {
    ...
    header.insertBefore(accent, header.querySelector('.meta-and-status'));
  }
  ...
  // Normalize the Items table so "Line Total" remains
...
const pdf = new jsPDFCtor({ unit: 'pt', format: 'letter', orientation: 'portrait' });
    ...
    const client = ($('[data-bind="client_name"]').value || 'Client').trim() || 'Client';
    const ino    = ($('[data-bind="invoice_no"]').value || 'Invoice').trim() || 'Invoice';
    const filename = `${client.replace(/[^\w\-]+/g,'_')}_${ino.replace(/[^\w\-]+/g,'_')}.pdf`;
    pdf.save(filename);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Recalc on inputs
  ['#discount-type', '#discount-value', '#tax-rate', '#fees', '#paid'].forEach(sel => {
    on(sel, 'input', recalcAll);
    on(sel, 'change', recalcAll);
  });

  // Dates affect status
  ['[data-bind="invoice_due"]', '[data-bind="invoice_date"]'].forEach(sel => {
    $(sel).addEventListener('input', () => updateStatusBadge());
    $(sel).addEventListener('change', () => updateStatusBadge());
  });
